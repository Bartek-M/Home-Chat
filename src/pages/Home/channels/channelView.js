        ...
        <>
            {!channel
                ...
                : <div className="main-view spaced-column-container">
                    ...
                    <div className="chat-window column-container scroller-container">
                        {(channel.messages && channel.messages.length)
                            ? <>
                                {channel.messages.map(message => (
                                    <li className="message-list-item container" key={Math.random()}>
                                        <img className="avatar" src={`/api/images/${message.author.avatar}.webp`} />
                                        <div className="message-content">
                                            <div className="message-info container">
                                                <p className="message-author">{message.author.name}</p>
                    ...
                    <div className="chat-inpt-wrapper container">
                        <div className="chat-inpt-scroller scroller-container">
                            <div className="chat-inpt" ref={text} autoFocus contentEditable onKeyDown={e => { if (!e.shiftKey && e.code === "Enter") e.preventDefault() }} onKeyUp={e => {
                                if (e.shiftKey || e.code !== "Enter" || !text.current.innerText || !text.current.innerText.trim()) return

                                socket.send({ channel_id: channel.id, author: user, content: text.current.innerText.trim() })
                        ...
                        <div className="chat-send-wrapper center-container">
                            ...
                            <button className="chat-send-btn center-container" onClick={e => {
                                if (!text.current.innerText || !text.current.innerText.trim()) return
                                socket.send({ channel_id: channel.id, author: user, content: text.current.innerText.trim() })
                                text.current.innerText = ""
                            }}>
