...
export function ChannelMenu({ element, x, y, close, setCard }) {
    ...
    return (
        <div className="channel-menu center-column-container" ref={menu} style={{ top: y + 20, left: x + 60, transform: "translateX(-95%)" }}>
            ...
            <button className="channel-menu-btn spaced-container" onClick={() => { setCard("channelMembers"); close() }}>
                Channel Members
                <svg width="16" height="16" fill="var(--FONT_RV_COLOR)" viewBox="0 0 16 16">
            ...
            {!channel.direct && (channel.admin || channel.owner === user.id) &&
                <button className="channel-menu-btn spaced-container" onClick={() => { setCard("channelInvite"); close() }}>
                    Invite Friends
                    <svg width="16" height="16" fill="var(--FONT_RV_COLOR)" viewBox="0 0 16 16">
