...
def handle_disconnect(client, person, name):
    """
    Handle client disconnect
    :param client: current client
    :param person: client person object
    :param name: client person name
    :return: None
    """
    client.close()
    ...
    while True: # wait for any messages from person
        try:
            ...
            if msg == bytes("{quit}", "utf8"): # if message is {quit} - disconnect client
                handle_disconnect(client, person, name)
                break
...
def wait_for_connection():
    ...
    print("[CRASHED] Server has crashed")
...
if __name__ == "__main__":
    ...
    ACCEPT_THREAD.join()
    SERVER.close()
    print("[STOPPED] Server has been stopped")