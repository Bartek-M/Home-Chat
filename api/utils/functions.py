...
class Functions:
    ...
    def update_notifications(db, sid, user_id, channel_id, last_message=None):
        if (
            not user_id
            or not channel_id
            or user_id not in rooms(sid)
            or channel_id not in rooms(sid)
            or not (user := db.get_entry(USER_TABLE, user_id))
            or not db.get_entry(CHANNEL_TABLE, channel_id)
        ...
        if (
            ...
            or not db.get_entry(USER_SETTING_TABLE, user_id).notifications_message
            or not user_channel.notifications
        ):
            return
