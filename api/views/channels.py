...
class Channels:
    ...
    def open_direct(db, user):
        ...
        socketio.emit(
            "member_list",
            {"channel_id": channel.get("id"), "member": user.__dict__, "action": "add"},
            to=channel.get("id"),
        )
    ...
    def send_message(db, user, channel_id):
        ...
        if user_channel.direct and (
            not (friend := db.get_user_stuff([user.id, channel_id.replace(user.id, "").replace("-", "")], "friend"))
            or friend.get("accepted", "waiting") == "waiting"
        ):
        ...
        if (
            user.notifications
            and db.get_entry(USER_SETTING_TABLE, user.id).notifications_message
            and user_channel.notifications
        ):
    ...
    def settings_channel(db, user, channel_id):
        ...
        if (name := request.json.get("name")) and name != channel.name:
            ...
            socketio.emit(
                "channel_change", {"channel_id": channel_id, "setting": "name", "content": name}, to=channel_id
            )
        ...
        if (nick := request.json.get("nick", "")) != user_channel.nick:
            ...
            socketio.emit(
                "member_change",
                {"channel_id": channel_id, "member_id": user.id, "setting": "nick", "content": nick},
                to=channel_id,
            )
        ...
        if notifications != user_channel.notifications:
            ...
            socketio.emit(
                "channel_change",
                {"channel_id": channel_id, "setting": "notifications", "content": notifications},
                to=user.id,
            )
    ...
    def member_nick(db, user, channel_id, member_id):
        ...
        if not (user_channel := db.get_channel_stuff([user.id, channel_id], "user_channel")) or not (
            member_channel := db.get_channel_stuff([member_id, channel_id], "user_channel")
        ):
        ...
        Functions.send_system_message(
            db, socketio, channel_id, f"{user.name} changed {member.name}'s nickname to '{nick}'"
        )
        socketio.emit(
            "member_change",
            {"channel_id": channel_id, "member_id": member_id, "setting": "nick", "content": nick},
            to=channel_id,
        )
        return 200
    ...
    def member_admin(db, user, channel_id, member_id):
        ...
        Functions.send_system_message(
            db, socketio, channel_id, f"{user.name} {'added' if admin_status else 'removed'} {member.name} as admin"
        )
        socketio.emit(
            "member_change",
            {"channel_id": channel_id, "member_id": member_id, "setting": "admin", "content": admin_status},
            to=channel_id,
        )
    ...
    def member_owner(db, user, channel_id, member_id):
        ...
        if not db.get_channel_stuff([user.id, channel_id], "user_channel") or not db.get_channel_stuff(
            [member_id, channel_id], "user_channel"
        ):
            return ({"errors": {"channel": "You or selected user is not a member"}}, 401)
        ...
        if (
            user_settings.mfa_enabled == 0
            and Security.hash_passwd(request.json.get("password"), user_secrets.password.split("$")[0])
        ...
        socketio.emit(
            "channel_change", {"channel_id": channel_id, "setting": "owner", "content": member_id}, to=channel_id
        )
        return 200
    ...
    def member_kick(db, user, channel_id, member_id):
        ...
        if not (
            user_channel := db.get_channel_stuff([user.id, channel_id], "user_channel")
        ) or not db.get_channel_stuff([member_id, channel_id], "user_channel"):
            return ({"errors": {"channel": "You or selected user is not a member"}}, 401)
        ...
        if socketio.server.manager.rooms["/"].get(member_id):
            socketio.emit(
                "member_list", {"channel_id": channel_id, "member": member_id, "action": "remove"}, to=member_id
            )
