...
class Recovery:
    ...
    def forgot_password(db):
        ...
        Mailing.send_password_recovery(
            user_settings.email, user.name, Security.gen_token(user.id, user_secrets.secret, "passw")
        )
    ...
    def no_mfa_access(db, user, option):
        ...
        Mailing.send_mfa_reset(
            user_settings.email, user.name, Security.gen_token(user.id, user_secrets.secret, "mfa-reset")
        )
        db.update_entry(USER_SECRET_TABLE, user.id, "sent_time", str(current_time))
        return 200
